...
const CouchbaseAdapter = require('../../src');
...
describe('Test CouchbaseAdapter', () => {
  beforeEach(() => {
    // Couchbase.mockClear();
  });

  const url = 'localhost:8091';
  const opts = {
    Username: 'admin',
    Password: '123456',
    bucketName: 'posts',
  };
  const adapter = new CouchbaseAdapter(url, opts);
  ...
  it('call connect with uri', () => {
    return adapter
      .connect('couchbase://127.0.0.1:8091')
      .catch(protectReject)
      .then(() => {
        expect(Couchbase).toHaveBeenCalledTimes(1);
  ...
  it('call find', () => {
    ...
    return adapter
      ...
      .then(() => {
        ...
        expect(adapter.db.find).toHaveBeenCalledWith({
          selector: params,
        });
      });
  });
  ...
  it('call insertMany', () => {
    ...
    return adapter
      ...
      .then(() => {
        ...
        expect(adapter.db.bulk).toHaveBeenCalledWith({
          docs: entities,
        });
      });
  });
